...
import io.github.alathra.alathranwars.database.handler.DatabaseType;
...
public class DatabaseConfig implements VersionedConfig {
    ...
    public @NotNull Map<Integer, Migration> migrations() {
        return Map.of(
            2, Migration.builder()
                // messaging.type was shipped with stale values ("sql", "plugin") that predate the
                // current BrokerType constants (DATABASE, PLUGIN_MESSAGING). Only rewrite those two
                // specific stale values; leave redis/rabbitmq/nats and anything already valid alone
                // so a server owner's actual broker choice is never clobbered.
                .withAction(NodePath.path("messaging", "type"), (path, node) -> {
                    final String raw = node.getString();
                    if ("sql".equalsIgnoreCase(raw))
                        node.raw("database");
                    else if ("plugin".equalsIgnoreCase(raw))
                        node.raw("plugin_messaging");
                    return null;
                })
                // messaging.address was shipped under the wrong key name; the field is
                // messaging.addresses (List<String>).
                .rename(NodePath.path("messaging", "address"), "addresses")
                .build()
        );
    }
    ...
    public static class Messaging {
        ...
        @Comment("Available broker types: \"database\", \"plugin_messaging\", \"redis\", \"rabbitmq\", \"nats\"")
