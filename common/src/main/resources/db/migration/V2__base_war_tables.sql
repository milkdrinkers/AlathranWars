...
CREATE TABLE IF NOT EXISTS "${tablePrefix}sides_spawns" (
        "side" BINARY(16) NOT NULL,
        "_name" TEXT NOT NULL,
        "last_proxied" TIMESTAMP NOT NULL,
        "start_proxied" TIMESTAMP NOT NULL,
        ...
        "block_world" BINARY(16) NOT NULL,
        "block_x" INT NOT NULL,
        "block_y" INT NOT NULL,
        "block_z" INT NOT NULL,
        FOREIGN KEY ("side") REFERENCES "${tablePrefix}sides"("uuid") ON DELETE CASCADE
);
CREATE INDEX "${tablePrefix}sides_spawns_side_index" ON "${tablePrefix}sides_spawns" ("side");
